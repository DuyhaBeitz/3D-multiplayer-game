...
Playground::Playground() : SceneRegular(0, heightmap_scale, trees_count, grass_count, 10.f, 5.0f) {
}
...
void Playground::UnloadResources() {
...
void Playground::Draw(const GameDrawingData &drawing_data) const {
...
GameState Playground::PopulateState(const GameState &old_state) {
    GameState state;
    ...
    for (int i = 1; i < 10; i++) {
        BoxData box_data;
        box_data.SetHalfExtends(Vector3{10, 10, 10});

        BodyData body_data;
        body_data.restitution = 0;
...
Scenes Playground::CheckSceneChange(const GameState &state) {
...
void Playground::InitNewPlayer(GameState &state, uint32_t id) {
