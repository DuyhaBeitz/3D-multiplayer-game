...
// the server runs in the past
// it broadcasts the world state a bit late
// to wait for late inputs and apply them retroactively
// in theory this is the maximum trip time (in ticks) that is still playable just fine
// a bit more and none of the client's inputs will be applied
// on practice it corresponds to round trip time, idk why
// this variable is also how much players will see each other in the past
...
/*
                                       'current_old_tick   'current_tick       'm_tick
ticks: |---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
*/
        ...
        if (m_tick % tick_period == 0 && m_tick >= max_lateness) {
            ...
            m_game_state = ApplyEvents(m_game_state, previous_tick, current_tick);
            ...
            DropEventHistory(previous_tick);
        }
